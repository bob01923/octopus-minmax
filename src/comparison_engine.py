...
class ComparisonResult:
    ...
    def should_switch(self) -> bool:
        is_target_switchable = self.cheapest_tariff is not None and self.cheapest_tariff.switchable
        logger.debug(
            f"current_tariff: {self.current_tariff_comparison.tariff.display_name}, "
            f"cheapest_tariff: {self.cheapest_tariff.display_name if self.cheapest_tariff else None}, "
            f"cheapest_switchable: {is_target_switchable}, "
            f"potential_savings: {self.potential_savings:.2f}p, "
            f"SWITCH_THRESHOLD: {config.SWITCH_THRESHOLD}p"
        )
        # We can switch FROM any tariff (including Intelligent Go),
        # but the TARGET tariff must be auto-switchable (excludes Intelligent Go and Flexible).
        return (is_target_switchable and
                self.cheapest_tariff != self.current_tariff_comparison.tariff and
                self.potential_savings > config.SWITCH_THRESHOLD)
...
class ComparisonEngine:
    ...
    def _find_best_option(self,
        ...
        """Find the absolute cheapest tariff option among all valid comparisons."""
        all_valid = [current] if current.is_valid else []
        all_valid.extend([comp for comp in alternatives if comp.is_valid])

        if not all_valid:
            return None, 0.0

        # Find the overall cheapest tariff
        cheapest = min(all_valid, key=lambda x: x.total_cost)

        # Calculate potential savings against current tariff
    ...
    def _calculate_potential_costs(self,
        ...
        for consumption in consumption_data:
            ...
            matching_rate = next((
                ...
                and rate.get('payment_method') in [None, "DIRECT_DEBIT"]
            ), None)

            # Fallback to general direct debit rate if exact boundary is missed
            if matching_rate is None:
                matching_rate = next((
                    rate for rate in rate_data
                    if rate.get('payment_method') in [None, "DIRECT_DEBIT"]
                ), rate_data[0] if rate_data else {'value_inc_vat': 0.0})
    ...
    def _get_potential_tariff_rates(self, tariff: Tariff, region_code: str) -> Tuple[float, list[dict], str]:
        ...
        url = f"{config.BASE_URL}/products/?brand=OCTOPUS_ENERGY&is_business=false"
        product = None

        def _is_product_match(p: dict) -> bool:
            if p.get('direction') != "IMPORT":
                return False
            name = p.get('display_name', '').strip().lower()
            target = tariff.api_display_name.strip().lower()
            if name == target:
                return True
            if tariff.id == "intelligent-go" and "intelligent" in name and "octopus" in name:
                return True
            return False

        # Query products (with pagination support)
        while url and not product:
            all_products = self.query_service.execute_rest_query(url)
            product = next((p for p in all_products.get('results', []) if _is_product_match(p)), None)
            url = all_products.get('next') if not product else None
        ...
        # Get standing charge including VAT
        ...
        unit_rates_link = next((
            ...
            if item.get('rel', '').lower() in ['standard_unit_rates', 'unit_rates']
        ), None)

        today = date.today()

        # If standard unit rates are linked, query them directly
        if unit_rates_link:
        ...
        # Fallback for two-rate EV tariffs (Go / Intelligent Go) listing day & night links
        day_link = next((item.get('href') for item in region_links if item.get('rel', '').lower() == 'day_unit_rates'), None)
        night_link = next((item.get('href') for item in region_links if item.get('rel', '').lower() == 'night_unit_rates'), None)

        if day_link and night_link:
            day_data = self.query_service.execute_rest_query(f"{day_link}?period_from={today}T00:00:00Z&period_to={today}T23:59:59Z")
            night_data = self.query_service.execute_rest_query(f"{night_link}?period_from={today}T00:00:00Z&period_to={today}T23:59:59Z")
            day_rate = day_data.get('results', [{}])[0].get('value_inc_vat', 0.0)
            night_rate = night_data.get('results', [{}])[0].get('value_inc_vat', 0.0)

            # Intelligent Go: 23:30 - 05:30 off-peak | Standard Go: 00:30 - 04:30 off-peak
            is_iog = 'intelligent' in tariff.id
            off_start = "23:30" if is_iog else "00:30"
            off_end = "05:30" if is_iog else "04:30"

            synthetic_rates = [
                {
                    "valid_from": f"{today}T00:00:00Z",
                    "valid_to": f"{today}T{off_end}:00Z",
                    "value_inc_vat": night_rate,
                    "payment_method": "DIRECT_DEBIT"
                },
                {
                    "valid_from": f"{today}T{off_end}:00Z",
                    "valid_to": f"{today}T{off_start}:00Z",
                    "value_inc_vat": day_rate,
                    "payment_method": "DIRECT_DEBIT"
                },
                {
                    "valid_from": f"{today}T{off_start}:00Z",
                    "valid_to": f"{today}T23:59:59Z",
                    "value_inc_vat": night_rate,
                    "payment_method": "DIRECT_DEBIT"
                }
            ]
            return standing_charge_inc_vat, synthetic_rates, product_code

        raise ValueError(f"No valid standard or day/night unit rates found for region: {region_code_key}")
    @property
    def is_valid(self) -> bool:
        return self.error is None and self.cost_breakdown is not None
...
class ComparisonResult:
    ...
    @property
    def should_switch(self) -> bool:
        logger.debug(f"current_tariff switchable: {self.current_tariff_comparison.tariff.switchable}, cheapest_tariff: {self.cheapest_tariff.display_name if self.cheapest_tariff else None}, potential_savings: {self.potential_savings}, SWITCH_THRESHOLD: {config.SWITCH_THRESHOLD}")
        return (self.current_tariff_comparison.tariff.switchable and
                self.cheapest_tariff is not None and
                self.cheapest_tariff != self.current_tariff_comparison.tariff and
